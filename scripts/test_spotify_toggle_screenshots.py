...
def expect_hidden(page, selector: str) -> None:
	...
	page.wait_for_function("sel => { const el = document.querySelector(sel); return !!el && getComputedStyle(el).display === 'none'; }", arg=selector)
...
def run() -> None:
	...
	try:
		...
		with sync_playwright() as p:
			...
			page.wait_for_function("() => document.getElementById('spotify-toggle')?.textContent?.includes('Disable Spotify Integration')")
			...
			page.wait_for_function("() => document.getElementById('spotify-toggle')?.textContent?.includes('Enable Spotify Integration')")
			expect_hidden(page, '#playlist-url-group')
