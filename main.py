...
if __name__ == "__main__":
    ...
    while not game.is_finished():
        ...
        print(
            f"{Fore.BLUE}Matchup 1:{Style.RESET_ALL} {format_contestant_with_points(state['pair_1'][0], game.leads + game.follows)} (Lead) & {format_contestant_with_points(state['pair_1'][1], game.leads + game.follows)} (Follow)"
        )
        print(
            f"{Fore.BLUE}Matchup 2:{Style.RESET_ALL} {format_contestant_with_points(state['pair_2'][0], game.leads + game.follows)} (Lead) & {format_contestant_with_points(state['pair_2'][1], game.leads + game.follows)} (Follow)"
        )
        print(
            f"{Fore.MAGENTA}Contestant Judges:{Style.RESET_ALL} {', '.join(state['contestant_judges'])}"
        )
        ...
        print(Fore.GREEN + f"Winner: {result_leads['winner']}")
        print(
            f"Guest Votes: {', '.join(result_leads['guest_votes']) if result_leads['guest_votes'] else 'None'}"
        )
        print(
            f"Contestant Votes: {', '.join(result_leads['contestant_votes']) if result_leads['contestant_votes'] else 'None'}"
        )
        ...
        result_follows = game.judge_round(
            game.pair_1[1], game.pair_2[1], "follow", votes
        )
        print(Fore.GREEN + f"Winner: {result_follows['winner']}")
        print(
            f"Guest Votes: {', '.join(result_follows['guest_votes']) if result_follows['guest_votes'] else 'None'}"
        )
        print(
            f"Contestant Votes: {', '.join(result_follows['contestant_votes']) if result_follows['contestant_votes'] else 'None'}"
        )
    ...
    sorted_leads = sorted(
        [lead for lead, _ in final_pairs], key=lambda l: l.points, reverse=True
    )
    sorted_follows = sorted(
        [follow for _, follow in final_pairs], key=lambda f: f.points, reverse=True
    )
