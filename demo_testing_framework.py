...
""")
    ...
    game = Game(["Lead1", "Lead2", "Lead3", "Lead4"], 
    ...
    for round_num in range(1, 4):
        print(f"\n  Round {round_num}:")
        print(f"    Competing: {transition_game.pair_1[0].name}-{transition_game.pair_1[1].name} vs {transition_game.pair_2[0].name}-{transition_game.pair_2[1].name}")
    ...
    for i in range(3):
        ...
        no_self_compete = (state.pair_1[0] != state.pair_2[0] and 
                          state.pair_1[1] != state.pair_2[1])
        ...
        points_ok = all(p >= 0 for p in state.lead_points.values()) and \
                   all(p >= 0 for p in state.follow_points.values())
